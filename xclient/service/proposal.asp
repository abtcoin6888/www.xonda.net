...
<head>
	...
	<title>쏜다넷 - 제안서/견적서신청 [대량문자,단체문자전송,단체문자사이트,문자전송,대량문자보내기,단체문자,단체문자발송,무료문자,문자사이트,문자발송발송사이트,인터넷으로문자보내기,문자보내기,대량문자전송,선거문자]</title>
	<meta name="title" content="쏜다넷 - 제안서/견적서신청 [대량문자,단체문자전송,단체문자사이트,문자전송,대량문자보내기,단체문자,단체문자발송,무료문자,문자사이트,문자발송발송사이트,인터넷으로문자보내기,문자보내기,대량문자전송,선거문자]">
	...
	<meta name="description" content="쏜다넷 - 제안서/견적서신청 [단체문자보내기,단체문자발송,문자사이트,SMS보내기,컴퓨터문자메세지보내기,웹발신문자,LMS문자,KT문자,SKT문자,LGT문자,그림문자,포토문자,WEB발신,SMS서비스,장문문자,엑셀문자,PC문자]">
	<meta name="Subject" content="쏜다넷 - 제안서/견적서신청 [대량문자,단체문자전송,단체문자사이트,문자전송,대량문자보내기,단체문자,단체문자발송,무료문자,문자사이트,문자발송발송사이트,인터넷으로문자보내기,문자보내기,대량문자전송,선거문자]">
	...
	<meta property='og:title' content='쏜다넷 - 제안서/견적서신청 [대량문자,단체문자전송,단체문자사이트,문자전송,대량문자보내기,단체문자,단체문자발송,무료문자,문자사이트,문자발송발송사이트,인터넷으로문자보내기,문자보내기,대량문자전송,선거문자]'>
	<meta property='og:description' content='쏜다넷 - 제안서/견적서신청 [단체문자보내기,단체문자발송,문자사이트,SMS보내기,컴퓨터문자메세지보내기,웹발신문자,LMS문자,KT문자,SKT문자,LGT문자,그림문자,포토문자,WEB발신,SMS서비스,장문문자,엑셀문자,PC문자]'>
...
<noscript>
	...
	<script type="text/javascript">
	/* proposal.min.js | Yoon | devholic.net */
function saveOK(){var e=document.frmInfo;if(0==e.chkAgree.checked)return makeAlert("개인정보보호를 위한 이용자 동의사항에 동의하셔야 합니다."),!1;if($("#companyname").pval().length<2)return makeAlert("회사명을 입력해 주십시요.",funcFocus(e.companyname)),!1;if($("#conname").pval().length<2)return makeAlert("담당자명을 입력해 주십시요.",funcFocus(e.conname)),!1;if(!validateEmail(e.email.pval()))return makeAlert("이메일 주소를 정확히 입력하세요.",funcFocus(e.email)),!1;if(!validateTel(e.telno1.pval(),e.telno2.pval(),e.telno3.pval()))return makeAlert("연락처 전화번호를 정확히 입력해주세요.<br><br>※ 지역번호가 없는 대표전화는 첫째칸을 생략하시면됩니다."),!1;if(!validateHP(e.hpno1.pval(),e.hpno2.pval(),e.hpno3.pval()))return makeAlert("휴대폰번호를 정확히 입력해주세요."),!1;if(""==$("#homepage").pval()&&(e.homepage.value="http://"),0==e.smsflag.checked&&0==e.lmsflag.checked&&0==e.mmsflag.checked)return makeAlert("전송형태를 1가지 이상 체크해 주십시요."),!1;if(1==e.smsflag.checked){if(""==e.smscnt.value||isNaN(e.smscnt.value))return makeAlert("단문 전송건수를 숫자로 입력해 주십시요.",funcFocus(e.smscnt)),!1;if(parseInt(e.smscnt.value)<1)return makeAlert("단문 전송건수를 1건이상 입력해 주십시요.",funcFocus(e.smscnt)),!1}if(1==e.lmsflag.checked){if(""==e.lmscnt.value||isNaN(e.lmscnt.value))return makeAlert("장문 전송건수를 숫자로 입력해 주십시요.",funcFocus(e.lmscnt)),!1;if(parseInt(e.lmscnt.value)<1)return makeAlert("장문 전송건수를 1건이상 입력해 주십시요.",funcFocus(e.lmscnt)),!1}if(1==e.mmsflag.checked){if(""==e.mmscnt.value||isNaN(e.mmscnt.value))return makeAlert("포토 전송건수를 숫자로 입력해 주십시요.",funcFocus(e.mmscnt)),!1;if(parseInt(e.mmscnt.value)<1)return makeAlert("포토 전송건수를 1건이상 입력해 주십시요.",funcFocus(e.mmscnt)),!1}makeConfirm("입력하신 정보로 신청하시겠습니까?",(function(){var e=serializeGETData("frmInfo"),a=execURL("POST","proposal_proc.asp",e),n=a.substring(0,4),t=a.substring(5);"0000"==n?""!=t?makeAlert(t,(function(){GoMain()})):GoMain():makeAlert("["+n+"] "+t)}))}
	...
	$(window).ready(function () {
		...
		$(document).keydown(function(e){
			...
			if(keyvalue == 8){
				if(!((eventtarget == "INPUT" && readonlycheck == false) || (eventtarget == "TEXTAREA" && readonlycheck == false))){
					makeConfirm("페이지를 벗어나시겠습니까?", function(){
						history.back();
					});
					e.keyCode = 0;
					return false;						
		...
		$("#smsflag").click(function(){
			if($(this).is(':checked'))
				$("#smscnt").attr("readonly",false);
			else
				$("#smscnt").attr("readonly",true);
		});
		$("#lmsflag").click(function(){
			if($(this).is(':checked'))
				$("#lmscnt").attr("readonly",false);
			else
				$("#lmscnt").attr("readonly",true);
		});
		$("#mmsflag").click(function(){
			if($(this).is(':checked'))
				$("#mmscnt").attr("readonly",false);
			else
				$("#mmscnt").attr("readonly",true);
		});
	});
	</script>
</head>
...
<div id="page-info">
	<!-- page title -->
	<h1 class="page-title">제안서/견적서신청</h1>
	...
	<div class="">
		...
		<div class="col-md-9 col-sm-12 col-xs-12">
			<div class="bs-callout bs-callout-danger">
				<h4 class="text-info">견적서신청</h4>
				<p><small>아래의 내용을 빠짐없이 작성하여 보내주시면 업무 담당자가 확인 후 빠른 시일내에 원하시는 견적서를 보내드리도록 하겠습니다.</small></p>
			</div>

			<div class="panel panel-danger marginB10">
				<div class="panel-heading panel-heading-sm">
					<h5 class="panel-title"><i class="fa fa-check-circle fa-lg marginR5"></i>개인정보보호를 위한 이용자 동의사항</h5>
				</div>
				<div class="panel-body panel-body-sm marginB10">
					<textarea class="form-control noresize f12px" name="textpolicy" placeholder="" rows="4" readonly>'(주)쏜다넷'은 (이하 '회사'는) 고객님의 개인정보를 중요시하며, "정보통신망 이용촉진 및 정보보호"에 관한 법률을 준수하고 있습니다.
...
(시행일) 이 약관은 2015년 08월 18일부터 시행합니다.
...
ο 수집항목 : 이름, 로그인ID, 비밀번호, 전화번호, 주소, 휴대전화번호, 이메일, 회사명, 부서, 직책, 회사전화번호, 서비스 이용기록, 접속 로그, 접속 IP 정보
ο 개인정보 수집방법 : 홈페이지(http://www.xonda.net) , 전화/팩스를 통한 회원가입 
...
원칙적으로, 개인정보 수집 및 이용목적이 달성된 후에는 해당 정보를 지체 없이 파기합니다. 단, 관계법령의 규정에 의하여 보존할 필요가 있는 경우 회사는 아래와 같이 관계법령에서 정한 일정한 기간 동안 회원정보를 보관합니다.
 ...
 보존 항목 : 이름, 로그인ID, 비밀번호, 전화번호, 주소, 휴대전화번호, 이메일, 회사명, 부서, 직책, 회사전화번호, 서비스 이용기록, 접속 로그, 접속 IP 정보
...
회사는 NICE 평가정보(주)에 본인인증 서비스를, NHN한국사이버결제에 결제시스템을 이용하고 있으며, 본인인증이 이루어지지 않았을 시에 이용에 제한이 있음을 알려드립니다.
...
회사는 귀하의 정보를 수시로 저장하고 찾아내는 ‘쿠키(cookie)’ 를 운용하지 않습니다.
...
4.경찰청 사이버테러대응센터 (www.ctrc.go.kr/02-392-0330)</textarea>
			...
			<form name="frmInfo" id="frmInfo" method="post" target="frameHidden" action="##gSITEDomainSSL##/xclient/service/proposal_proc.asp" class="form-horizontal" role="form">
			<div>
				<label class="f12px fNormal"><input type="checkbox" name="chkAgree" id="chkAgree" title="동의합니다" value="Y"> 위 약관에 동의합니다.</label>
				<button type="button" class="btn btn-warning btn-xs pull-right" onclick="viewPrivacy()"><i class="fa fa-search marginR5"></i>크게보기</button>
			</div>

			<div class="panel panel-success marginT20">
				<div class="panel-heading panel-heading-sm">
					<h5 class="panel-title"><i class="fa fa-check-circle fa-lg marginR5"></i>신청서작성</h5>
				</div>
				<div class="panel-body f12px">
					<div class="form-group form-group-sm">
						<label for="cusflag" class="col-sm-2 col-xs-12 control-label">구분<span class="required"></span></label>
						<div class="col-sm-6 col-xs-12">
							<label for="gubun1" class="fNormal">
								<input type="radio" name="gubun" id="gubun1" value="1" checked /> 법인사업자
							</label> 
							<label for="gubun2" class="fNormal marginL20">
								<input type="radio" name="gubun" id="gubun2" value="2" /> 개인사업자
							</label>
						</div>
					</div>
					<div class="form-group form-group-sm">
						<label for="username" class="col-sm-2 col-xs-12 control-label">회사명<span class="required"></span></label>
						<div class="col-sm-6 col-xs-12">
							<input type="text" name="companyname" class="form-control" id="companyname" value="" maxlength="25" placeholder="회사명을 입력 하세요" required>
						</div>
					</div>
					<div class="form-group form-group-sm">
						<label for="ceoname" class="col-sm-2 col-xs-12 control-label">담당자<span class="required"></span></label>
						<div class="col-sm-6 col-xs-12">
							<input type="text" name="conname" class="form-control" id="conname" value="" maxlength="10" placeholder="담당자명을 입력 하세요" required>
						</div>
					</div>
					<div class="form-group form-group-sm">
						<label for="email" class="col-sm-2 col-xs-12 control-label">이메일<span class="required"></span></label>
						<div class="col-sm-10 col-xs-12">
							<input type="email" name="email" class="form-control" id="email" value="" maxlength="30" placeholder="이메일을 입력 하세요." required>
						</div>
					</div>
					<div class="form-group form-group-sm">
						<label for="telno1" class="col-sm-2 col-xs-12 control-label">연락처<span class="required"></span></label>
						<div class="col-sm-6 col-xs-12">
							<div class="input-group">
								<input type="tel" name="telno1" class="form-control" id="telno1" value="" maxlength="3" placeholder="지역번호" required>
								<div class="input-group-addon paddingL5 paddingR5">-</div>
								<input type="tel" name="telno2" class="form-control" id="telno2" value="" maxlength="4" placeholder="국번" required>
								<div class="input-group-addon paddingL5 paddingR5">-</div>
								<input type="tel" name="telno3" class="form-control" id="telno3" value="" maxlength="4" placeholder="개별번호" required>
							</div>
						</div>
					</div>
					<div class="form-group form-group-sm">
						<label for="hpno1" class="col-sm-2 col-xs-12 control-label">휴대전화<span class="required"></span></label>
						<div class="col-sm-6 col-xs-12">
							<div class="input-group">
								<input type="tel" name="hpno1" class="form-control" id="hpno1" value="" maxlength="3" placeholder="" required>
								<div class="input-group-addon paddingL5 paddingR5">-</div>
								<input type="tel" name="hpno2" class="form-control" id="hpno2" value="" maxlength="4" placeholder="" required>
								<div class="input-group-addon paddingL5 paddingR5">-</div>
								<input type="tel" name="hpno3" class="form-control" id="hpno3" value="" maxlength="4" placeholder="" required>
							</div>
						</div>
					</div>
					<div class="form-group form-group-sm">
						<label for="homepage" class="col-sm-2 col-xs-12 control-label">홈페이지</label>
						<div class="col-sm-10 col-xs-12">
							<input type="text" name="homepage" class="form-control" id="homepage" value="http://" maxlength="50" placeholder="홈페이지 URL을 입력하세요.">
						</div>
						<div class="col-sm-4 col-xs-3 paddingL0"></div>
					</div>
					<div class="form-group form-group-sm">
						<label for="cusflag" class="col-sm-2 col-xs-12 control-label">전송형태<span class="required"></span></label>
						<div class="col-sm-10 col-xs-12">
							<label for="smsflag" class="fNormal">
								<input type="checkbox" name="smsflag" id="smsflag" value="1" /> 단문
							</label> 
							<label for="lmsflag" class="fNormal marginL30">
								<input type="checkbox" name="lmsflag" id="lmsflag" value="1" /> 장문
							</label>
							<label for="mmsflag" class="fNormal marginL30">
								<input type="checkbox" name="mmsflag" id="mmsflag" value="1" /> 포토
							</label>
						</div>
					</div>
					<div class="form-group form-group-sm">
						<label for="homepage" class="col-sm-2 col-xs-12 control-label">월예상건수<span class="required"></span></label>
						<div class="col-sm-3 col-xs-12">
							<div class="input-group input-group-sm">
								<div class="input-group-addon">단문</div>
								<input type="number" name="smscnt" class="form-control text-center" style="padding:0" id="smscnt" value="0" maxlength="7" placeholder="" readonly required>
								<div class="input-group-addon">건</div>
							</div>
						</div>
						<div class="col-sm-3 col-xs-12">
							<div class="input-group input-group-sm">
								<div class="input-group-addon">장문</div>
								<input type="number" name="lmscnt" class="form-control text-center" style="padding:0" id="lmscnt" value="0" maxlength="7" placeholder="" readonly required>
								<div class="input-group-addon">건</div>
							</div>
						</div>
						<div class="col-sm-3 col-xs-12">
							<div class="input-group input-group-sm">
								<div class="input-group-addon">포토</div>
								<input type="number" name="mmscnt" class="form-control text-center" style="padding:0" id="mmscnt" value="0" maxlength="7" placeholder="" readonly required>
								<div class="input-group-addon">건</div>
							</div>
						</div>
					</div>
					<div class="form-group form-group-sm">
						<label for="homepage" class="col-sm-2 col-xs-12 control-label">기타</label>
						<div class="col-sm-10 col-xs-12">
							<textarea class="form-control noresize" name="etc" placeholder="" rows="3"></textarea>
						</div>
					</div>
				</div>
			</div>
			<div class="text-center">
					<button type="button" class="btn btn-success" onclick="saveOK()">신청하기</button>
					<button type="reset" class="btn btn-danger marginL5">다시쓰기</button>
			</div>
			</form>
		</div>
	</div>
		...
		<div class="modal-dialog type-primary">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<input type="hidden" name="nexturl" value="/xclient/service/proposal.asp">
